...
@Mixin(value = EntityLanderBase.class, remap = false)

public abstract class MixinEntityLanderBase extends EntityAdvancedMotion
                                            implements IInventorySettable, IScaleableFuelLevel {

    private final int FUEL_TANK_CAPACITY = 50000;
    ...
    public MixinEntityLanderBase(World var1) {
        super(var1);
    ...
    public void setPositionAndRotationDirect(double x, double y, double z, float yaw, float pitch,
                                             int posRotationIncrements, boolean b) {
        ...
        if (this.syncAdjustFlag && this.world.isBlockLoaded(new BlockPos(x, 255D, z))) {
            ...
            if (dx * dx + dz * dz < 1024) {
                ...
                if (!this.addedToChunk || this.chunkCoordX != cx || this.chunkCoordZ != cz) {
                    if (this.addedToChunk && this.world
                            .isBlockLoaded(new BlockPos(this.chunkCoordX << 4, 255, this.chunkCoordZ << 4), true)) {
                        this.world.getChunk(this.chunkCoordX, this.chunkCoordZ).removeEntityAtIndex(this,
                                this.chunkCoordY);
                    }
    ...
    public MixinEntityLanderBase(World var1, double var2, double var4, double var6, float yOffset) {
        this(var1);
        this.setPosition(var2, var4, var6);
    }

    public MixinEntityLanderBase(EntityPlayerMP player, float yOffset) {
                        ...
                        if (e != null) {
                            if (e.dimension != this.dimension) {
                                if (e instanceof EntityPlayer) {
                                    e = WorldUtil.forceRespawnClient(this.dimension, e.world.getDifficulty().getId(),
                                            e.world.getWorldInfo().getTerrainType().getName(),
